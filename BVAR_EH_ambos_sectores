...
series <- c("exportaciones_primarias","itcer","ipi_eeuu", 
            "iti", "exportaciones_manofactura", "remesas", 
            "no_transable")
...
#aplicar Primera diferencias a algunas series 
...
resultados$d1_export_mano <- verificar_estacionariedad(
  data = endogenas_originales,
  nombre_col = "exportaciones_manofactura",
...
resultados$d1_export_primaria <- verificar_estacionariedad(
  data = endogenas_originales,
  nombre_col = "exportaciones_primarias",
...
p3 <- plot_series_professional(data_final, "d1_no_transable", "Primera diferencia del Sector No Transable")
...
plot_irf <- function(df) {
  ...
  labels <- c(
    ...
    d1_no_transable = "Sector No Transable",
    d1_iti = "ITI"
  )
...
combined_11 <- (plots_11[[1]] | plots_11[[2]]) /
  ...
  plot_annotation(
    ...
    caption  = paste0("Horizonte: ", n_horizon, " Trimestres | Periodo: 2006-2023\n",
                      "Fuente: BCN, SECMCA & FRED| Elaboración propia estimada con modelo BVAR"),
    theme = theme(
...
#lista combinada
todos_los_irfs_ambos <- do.call(rbind, irf_combinados_ambos)

#excel
hoja_irf <- list(
  "Irfs_Total" = todos_los_irfs_ambos           #todos los irfs
)

#exportarlo
write.xlsx(hoja_irf, file = "irf_total(ambos_modelos.xlsx")
...
make_irf_table("d1_remesas", "d1_itcer",
               "tabla_irf_remesa_itcer.tex",
               "Irf Chock Remesa afecta a ITCER")

make_irf_table("d1_itcer", "d1_transable",
               "tabla_irf_itcer_transab;e.tex",
               "Irf Chock ITCER afecta al Sector Transable")

make_irf_table("d1_iti", "d1_itcer",
               "tabla_irf_iti_itcer.tex",
               "Irf Chock ITI afecta a ITCER")

make_irf_table("d1_itcer", "d1_no_transable",
               "tabla_irf_itcer_exp_primaria.tex",
               "Irf Chock ITCER afecta al Sector No Transable")
...
irf_list_acum <- list(
  ...
  get_irf_acumulada(irf_all, "d1_itcer", "d1_no_transable"),
...
# 2do plot
combined_acum_22 <- (plots_acum11[[4]] | plots_acum11[[5]]) +
  plot_annotation(
...
#lista usada 
irf_combinados_ambos_acum <- list(
  get_irf_acumulada(irf_all, "d1_remesas", "d1_itcer"),
  get_irf_acumulada(irf_all, "d1_itcer", "d1_no_transable"),
  get_irf_acumulada(irf_all, "d1_itcer", "d1_transable"),
  get_irf_acumulada(irf_all, "d1_ipi_eeuu", "d1_itcer"),
...
#lista combinada
todos_los_irfs_ambos_acum <- do.call(rbind, irf_combinados_ambos_acum)

#excel
hoja_irf_acum <- list(
  "Irfs_Total" = todos_los_irfs_ambos_acum           #todos los irfs
)

#exportarlo
write.xlsx(hoja_irf_acum, file = "irf_total_acum(ambos_modelos.xlsx")
...
make_irf_acum_table("d1_remesas", "d1_itcer",
                    "tabla_irf_remesa_itcer.tex",
                    "Irf Chock Remesa afecta a ITCER")

make_irf_acum_table("d1_itcer", "d1_transable",
                    "tabla_irf_itcer_transable.tex",
                    "Irf Chock ITCER afecta al Sector Transable")

make_irf_acum_table("d1_itcer", "d1_no_transable",
                    "tabla_irf_itcer_no_transable.tex",
                    "Irf Chock ITCER afecta al Sector No Transable")

make_irf_acum_table("d1_iti", "d1_itcer",
                    "tabla_irf_iti_itcer.tex",
                    "Irf Chock ITI afecta a ITCER")
...
shock_index_no_transable <- which(variables == "d1_no_transable")
...
#exportar a excel
shocks <- c("d1_remesas", "d1_itcer", "d1_transable", "d1_no_transable")
sheets <- list()
